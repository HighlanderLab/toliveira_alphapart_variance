...
for (generation in seq(nburnin + 1, nburnin + nfuture, 1)) {
  ...
  SelMales <- 
    ...
    selectInd(nInd = nSires, use = "pheno")
  ...
  SelF <- selectInd(SelFemales, nDams/2, use = "pheno")
...
# Save results
#-----------------------------------------------------------------------
saveRDS(RecSys, file="./Results/Scenario3.rds")
saveRDS(RecVar, file="./Results/Scenario3Var.rds")